...
- **Resources are generated by DATAGEN into `common/src/main/resources`.** A single NeoForge `runData`
  harness (in `neoforge/`, driving shared common `DataProvider`s) writes the loader-agnostic
  blockstates/models/loot/recipes/tags/lang JSON that all six cells consume; run it with
  `:neoforge:<mc>:runData` (via the gradle MCP on the developer's machine — the agent sandbox can't run
  the game). Textures come from `tools/gen_textures.py` (see [`MODELS.md`](MODELS.md)); a few genuinely
  bespoke resources may still be hand-authored. Validate JSON after any edit or regeneration.
