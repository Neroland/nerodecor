#!/usr/bin/env python3
"""
Generate the NERODECOR mod logo (square), in the shared Neroland family style
...
the decoration/furniture mod, so the core-prism is set in an ornamental ROSE frame with paint-dab accents Renders supersampled, then downsamples.

Outputs:
  art/logo/nerodecor_logo.png       (1024x1024 master)
  art/logo/nerodecor_logo_400.png   (CurseForge/Modrinth-ready)
  common/src/main/resources/nerodecor_logo.png  (256x256 in-game mods-list icon)
"""
SEED = 53
MOD = 'nerodecor'
NAME = 'NERODECOR'
ACCENT = (232, 120, 142)
ACCENT_BRIGHT = (255, 176, 194)
BRIGHT = (255, 232, 238)
NEBULA = (112, 44, 62)
STAR_TINT = (255, 206, 218)
NAME_GLOW = (232, 120, 142, 255)
TAG_COL = (236, 190, 200, 255)
...
TAG = 'F U R N I T U R E   ·   D E C O R   ·   S T Y L E'

def emblem_frame(base, cx, cy, rad):
    # rounded ornamental frame
    gl = Image.new("RGBA", (R, R), (0, 0, 0, 0)); gd = ImageDraw.Draw(gl)
    s = rad * 1.72
    gd.rounded_rectangle([cx - s, cy - s, cx + s, cy + s], radius=int(rad * 0.7),
                         outline=ACCENT + (230,), width=SS * 3)
    gd.rounded_rectangle([cx - s * 0.86, cy - s * 0.86, cx + s * 0.86, cy + s * 0.86],
                         radius=int(rad * 0.55), outline=ACCENT + (150,), width=SS)
    # corner flourishes
    for sx in (-1, 1):
        for sy in (-1, 1):
            ax, ay = cx + sx * s, cy + sy * s
            gd.arc([ax - rad * 0.5, ay - rad * 0.5, ax + rad * 0.5, ay + rad * 0.5],
                   start=0, end=360, fill=ACCENT_BRIGHT + (220,), width=SS * 2)
    base.alpha_composite(gl)
    # palette dabs
    nl = Image.new("RGBA", (R, R), (0, 0, 0, 0)); nd = ImageDraw.Draw(nl)
    for t, col in ((210, NERO_ALLOY), (330, PLASMA), (90, ACCENT)):
        a = math.radians(t); nx, ny = cx + math.cos(a) * rad * 1.72, cy + math.sin(a) * rad * 1.72
        dr = rad * 0.2
        nd.ellipse([nx - dr, ny - dr, nx + dr, ny + dr], fill=col + (255,), outline=(240, 240, 250, 220), width=SS)
